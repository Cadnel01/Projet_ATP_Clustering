...
df_clean %>% 
  ggplot()+geom_boxplot(mapping = aes(x = as.character(df_clean$classe_hc), y = ace,
                                      fill = as.character(df_clean$classe_hc) ))
```
Ces joueurs réalisent en moyenne 13 services gagnants par match ce qui est inférieure à la moyenne du cluster 3
et supérieure aux moyennes de services gagnants des cluster 1 et 4
...
avec une moyenne d'environ 12 par match ce qui est nettement supérieures aux moyennes des autres clusters.
...
df_clean %>% 
  ggplot()+geom_boxplot(mapping = aes(x = as.character(df_clean$classe_hc), y = bpFaced,
                                      fill = as.character(df_clean$classe_hc) ))
```
Les joueurs gagnent un maximun de break point face à leurs adversaires, avec en moyenne 20 break point gagné par match ce qui les classe à la tete de cette catégorie en les comparant aux autres clusters.

En définitive, les joueurs de ce cluster ont l'avantage au niveau des break points sauvés et gagnés
Ensuite ils gagnent généralement le 1er point à partir du 1er service ce qui justifie leur reussite
...
acp[3]$ind$contrib
```
Au total les joueurs de ce cluster réalisent d'enormes performances au niveau des (stin) nombre de services gagnés au 1er coup et des (stwon) nombre de fois à avoir gagné le 1er point du 1er coup.
On a donc des joueurs qui sont très bons en service et gagnent d'importantes balles de matchs. 
...
Base4 <- Base4[,-c(9,10)]
```
le 4 ème groupe  contient ,`r round(nrow(Base4)/nrow(df_clean)*100,2)`% observation Les joueurs les plus connus,dans ce cluster sont nottament, Nadal, Wavrinka
Les joueurs qui composes ce groupe, ont en moyenne 27 ans. Pour la plus part ils sont donc expérimenté dans ce sport. Les joueurs font en moyen `r round(mean(Base4$height),2)` cm. considérer a d'autre cluster, ce groupes a une taille plus faible en moyenne.

En moyenne, tous les joueurs de ce groupe initatelement sont généralement bon en service, en effet ils réussisent en genéral tous leurs serives par ailleurs, on constate par ailleurs que ces jouerus utilise ces attouts pour gagner leurs premier dès le services. Par ailleurs, ces joueurs sont très fort sur les balle de break car lorsqu'ils ont a faire face a une balle de break, 75% du temps ils arrivent a la sauvé. Par ailleurs le graphique (annexe 5) permet de montrer la corélation entre le nombre de break auquel les joueurs et le nombre de break qu'ils ont gagné ainsi on aperçoit clairement que lorsque le nombre de break auquel ils font fasse augmente, il est plus probable que ces dernier arrive à la sauvé.
Par ailleurs, ce groupe joue en moyenne 3h41 en moyenne, Ceci s'explique par la présence de grand joueurs tels que:

- Nadal
- Monfils
- Lewitt ...

Ces joueurs participe au grand chellems qui se jouent sur 3 sets et non 2. Ainsi cela signifie qui doivent jouer plus que la moyenne.

La présene d'Hernych et  Denis Gremelmayr sont a douter. En effet, Ces 2 joueurs ne correspondent pas aux critère cité plus haut.(annexe 6) ils doivent jouer probablement avec un autre style de jeu.
...
# Corelation entre le nombre de break auquel ils ont fait face et sauvé
```{r Graph 4 cluster, echo = FALSE, eval = TRUE}
...
## ACP pour le cluster 4

```{r}
a<-FactoMineR::PCA(Base4,scale.unit = T)
fviz_pca(a)
```
L'ACP permet de confirmer que les 2 joureurs cité plus haut ne font pas partie du bon groupe. En effet ces dernier peuvent être considerer comme des individus suplémentaire. Par ailleurs la contribution de chacun s'élévent à 0.0194 et 0.0549, ceux qui est très faible 
