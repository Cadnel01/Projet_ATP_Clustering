...
```{r, include = FALSE}
setwd('C:/Users/Leon/Documents/M2/SEP0942_Regnault')
...
De plus, les joueurs réussissent en moyenne 61% de leurs premiers services, ce qui est équivalent aux autres clusters. Cependant, ce groupe se positionne à la tête des clusters en terme de gain du point après le premier échange. Les joueurs de ce cluster s'illustrent donc avec une moyenne d'environ 85 par match, ce qui est clairement supérieur aux moyennes des autres clusters. De même, ils réalisent 11% d'aces parmi leurs services réussis, ce qui est le deuxième meilleur taux parmis nos clusters. Enfin, dans la catégorie des points de break sauvés, ce cluster fait encore la différence avec 63% de breaks sauvés en moyenne par match, ce qui est supérieur aux moyennes des autres clusters.
